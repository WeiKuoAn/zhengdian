...
Route::middleware(['auth'])->group(function () {
    ...
    /* 官網內容管理（僅管理者 level 0、1） */
    Route::middleware('landing.admin')->group(function () {
        Route::get('landing', [LandingAdminController::class, 'index'])->name('landing.dashboard');
        Route::get('landing/sections', [LandingSectionController::class, 'show'])->name('landing.sections');
        Route::post('landing/sections/{section}', [LandingSectionController::class, 'update'])->name('landing.sections.update');
        Route::redirect('landing/settings', '/landing/sections?section=seo');
        Route::get('landing/content-items', function (\Illuminate\Http\Request $request) {
            $type = (string) $request->input('type', 'stat');

            return redirect()->route('landing.sections', [
                'section' => \App\Support\LandingSectionRegistry::sectionForContentType($type),
            ]);
        })->name('landing.content-items');
