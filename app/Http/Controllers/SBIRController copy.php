...
class SBIRController extends Controller
{
    ...
    public function export($id): string
    {
        ...
        // 2. **先** 解壓模板到暫存，保證後續圖片可用
        $templatePath = storage_path('app/templates/sbir05.docx');
        $this->tempDir = sys_get_temp_dir() . '/sbir05_' . uniqid();
        ...
        // 4. 讀取 document.xml
        $docXmlPath  = $this->tempDir . '/word/document.xml';
        ...
        $replace = [
            $wordXml1,$wordXml2,$wordXml3,$wordXml4,$wordXml5,$wordXml6,$wordXml7,$wordXml8,$wordXml9,$wordXml10,$wordXml11,$wordXml12,$wordXml13,
        ];
        $documentXml = str_replace($search, $replace, $documentXml);
        File::put($docXmlPath, $documentXml);

        // 6. 新增：寫入圖片關係
        $this->writeImageRelationships($this->tempDir);
        ...
        foreach ($files as $file) {
            if (!$file->isDir()) {
                $relativePath = substr($file->getRealPath(), strlen($this->tempDir) + 1);
    ...
    private function convertParagraph($node)
    {
        ...
        foreach ($node->childNodes as $child) {
            if ($child->nodeName === 'br') {
                ...
                // 段落內 img
                $paragraphXml .= $this->convertImage($child);
        ...
        return <<<XML
<w:p><w:pPr><w:ind w:left="1100"/></w:pPr>{$paragraphXml}</w:p><w:p><w:pPr><w:ind w:left="1100"/></w:pPr><w:r><w:t xml:space="preserve"></w:t></w:r></w:p>
XML;
    }
    ...
    private function convertImage(\DOMElement $node): string
    {
        ...
        $relative = parse_url($src, PHP_URL_PATH);
        $local    = public_path($relative);
        if (!file_exists($local)) return '';

        $this->imageCounter++;
        $ext       = pathinfo($local, PATHINFO_EXTENSION);
        $imageName = "image{$this->imageCounter}.{$ext}";
        copy($local, "{$this->tempDir}/word/media/{$imageName}");

        $rId = "rIdImage{$this->imageCounter}";
        $this->imageRelationships[] = ['Id' => $rId, 'Target' => "media/{$imageName}"];

        $w = intval($node->getAttribute('width')) * 9525;
        $h = intval($node->getAttribute('height')) * 9525;

        return <<<XML
<w:p><w:r><w:drawing><wp:inline><wp:extent cx="{$w}" cy="{$h}"/><a:graphic xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main"><a:graphicData uri="http://schemas.openxmlformats.org/drawingml/2006/picture"><pic:pic xmlns:pic="http://schemas.openxmlformats.org/drawingml/2006/picture"><pic:blipFill><a:blip r:embed="{$rId}" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"/></pic:blipFill></pic:pic></a:graphicData></a:graphic></wp:inline></w:drawing></w:r></w:p>
XML;
    }

    // === 新增：寫入圖片關係 ===
    private function writeImageRelationships(string $dir): void
    {
        // document.xml.res
        $relsPath = "{$dir}/word/_rels/document.xml.rels";
        $relsXml  = File::get($relsPath);
        $insert   = '';
        foreach ($this->imageRelationships as $rel) {
            $insert .= "<Relationship Id=\"{$rel['Id']}\" Type=\"http://schemas.openxmlformats.org/officeDocument/2006/relationships/image\" Target=\"{$rel['Target']}\"/>";
        }
        $relsXml = str_replace('</Relationships>', $insert . '</Relationships>', $relsXml);
        File::put($relsPath, $relsXml);

        // [Content_Types].xml
        $ctPath = "{$dir}/[Content_Types].xml";
        $ctXml  = File::get($ctPath);
        foreach ($this->imageRelationships as $rel) {
            $ext     = pathinfo($rel['Target'], PATHINFO_EXTENSION);
            $ctype   = $ext === 'png' ? 'image/png' : 'image/jpeg';
            $override = "<Override PartName=\"/word/{$rel['Target']}\" ContentType=\"{$ctype}\"/>";
            $ctXml    = str_replace('</Types>', $override . '</Types>', $ctXml);
        }
        File::put($ctPath, $ctXml);
    }
